    ...
    public boolean handleCommand(final CommandSender sender, final org.bukkit.command.Command cmd, final String label, final String[] args) {
        for(int i = args.length; i >= 0; i--) {
            ...
            if(commandMap.containsKey(cmdLabel)) {
                ...
                if(command.requiresFaction()) {
                    final Player player = (Player) sender;
                    final Profile profile = ProfileManager.get().get(player.getUniqueId());
                    if(!profile.hasFaction()) {
                        player.sendMessage(Locale.COMMAND_FACTION_REQUIRES_FACTION.toString());
                        return true;
                    }

                    if(command.requiredRank() != FactionMember.Rank.MEMBER) {
                        final FactionMember member = profile.getFactionObj().getMember(profile.getUuid());
                        final FactionMember.Rank requiredRank = command.requiredRank();
                        if(!member.isAtLeast(requiredRank)) {
                            player.sendMessage(Locale.COMMAND_FACTION_REQUIRES_RANK.toString().replace("%rank%", requiredRank.getFriendlyName()));
                            return true;
                        }
                    }
                }
